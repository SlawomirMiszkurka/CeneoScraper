...
while(page_no):
    ...
    if  response.status_code == requests.codes.ok:
        ...
        for review in reviews:
            ...
            publish_date = extract_element(review,"span.user-post__published > time:nth-child(1)")
            purchase_date = extract_element(review,"span.user-post__published > time:nth-child(2)","datetime")
            useful = extract_element(review,"span[id^=vote-yes]")
            useless = extract_element(review"span[id^=vote-no]")
